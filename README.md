...
# 前台运行（调试用）
./docker-proxy-linux-amd64 -addr :5001

# 后台运行
./docker-proxy-linux-amd64 -d -addr :5001
...
[Service]
Type=simple
ExecStart=/opt/docker-proxy/docker-proxy-linux-amd64 -addr :5001
Restart=always
RestartSec=5
...
```json
{
  "registry-mirrors": ["http://你的服务器IP:5001"],
  "insecure-registries": ["你的服务器IP:5001"]
}
```
...
```bash
docker pull 你的服务器IP:5001/library/nginx:latest
docker pull 你的服务器IP:5001/bitnami/redis:latest
```

拉取后可用 `docker tag` 重命名：

```bash
docker tag 你的服务器IP:5001/library/nginx:latest nginx:latest
```
...
```bash
curl -s http://你的服务器IP:5001/v2/library/alpine/manifests/latest \
